TimedText = (function(){
    ...
    var TimedText,
        ...
        mime_types = {},
        mimeexp = /([^\s()<>\[\]@,;:"\/\\?.=]+\/[^\s()<>\[\]@,;:"\/\\?.=]+)(;.*)?/,
        r_exp = /[\u05E0\u07E0\u05E1\u07E1\u05E2\u07E2\u05E3\u07E3\u05E4\u07E4\u05E5\u07E5\u05E6\u07E6\u05E7\u07E7\u05E8\u07E8\u05E9\u07E9\u05BE\u05C0\u05C3\u05C6\u05D0-\u05DF\u05EA\u05F0-\u05F4\u07C0-\u07DF\u07EA\u07F4\u07F5\u07FA\u200F\uFB1D\uFB1F-\uFB28\uFB2A-\uFB36\uFB38-\uFB3C\uFB3E\uFB40\uFB41\uFB43\uFB44\uFB46-\uFB4F]/,
    ...
    function convertUnits(value, target, prec) {
        ...
        if(!units || units === target){ return number; }
        ...
        if(target === 'px'){
            ...
            d.style.width = "1"+target;
            tpx = parseFloat(getComputedStyle(d).width);
            res = number/tpx;
        }else{
            d.style.width = "1"+units;
            upx = parseFloat(getComputedStyle(d).width);
    ...
    function assert_support(mime){
        ...
        if(!mime_types.hasOwnProperty(mime)){ throw new Error('Unsupported Mime-Type'); }
        return mime;
    }

    function getExt(mime){
        return mime_types[assert_support(mime)].extension;
    }

    function getTypeName(mime){
        return mime_types[assert_support(mime)].name;
    }

    function getCueMime(cue){
        var i, mime,
            constructor = cue.constructor,
            mimes = Object.keys(mime_types);
        for(i = 0; mime = mimes[i]; i++){
            if(mime_types[mime].cueType === constructor){ return mime; }
        }
        return "application/octet-stream";
    };

    function getTrackMime(track){
        if(!track.cues.length){ return "application/octet-stream"; }
        return getCueMime(track.cues[0]);
    };

    function getCueType(mime){
    ...
    function getCueConverter(from, to){
        ...
        return (typeof converters === 'object' && typeof converters[from] === 'function')
             ? converters[from] : defaultConverter.bind(toType.cueType);
    }

    function getTypeInfo(mime){
        return mime_types[assert_support(mime)];
    }

    function getCueTypeInfo(cue){
        var mime, type;
        for(mime in mime_types){
            if(!mime_types.hasOwnProperty(mime)){ continue; }
            type = mime_types[mime];
    ...
    TimedText = {
        registerType: function(mime, opts){
            mime_types[strip_mime(mime)] = opts;
        },
        extendType: function(mime, opts){
            mime = strip_mime(mime);
            if(!mime_types.hasOwnProperty(mime)){ mime_types[mime] = opts; }
            else{ merge_objects(mime_types[mime], opts); }
        },
        ...
        getCueType: getCueType,
        getCueMime: getCueMime,
        getTrackMime: getTrackMime,
        getCueConverter: getCueConverter,
        getCueTypeInfo: getCueTypeInfo,
        getExt: getExt,
